...
- [ ] **Two tests cannot share a name**
  - *Done when:* `speq validate` reports duplicate test names the way it
    reports duplicate step ids.
  - *Where:* `packages/core/src/validate.ts` checks step ids and says nothing
    about names, while every event is keyed by name.
  - *Why here:* it is half an hour of work today and a prerequisite for
    parametrization, where names are generated by a machine rather than typed
    by an author.
